...
int main()
{
	...
	while(getline(file, user))
	{
		...
		myHashTable->addEntry(user, salt, sha256(pwd+salt));
	}}
	...
	do
	{
		...
		switch(menuChoice)
		{
			case 1:	cout << "enter your username:  ";
					...
					if(myHashTable->validateLogin(username, sha256((password) + (myHashTable->getSalt(username)))))//check if the user's credentials are correct-----------------------------------
			...
			case 2: cout << "enter your new username:  ";
					...
					salt = myHashTable->generateSalt();
					myHashTable->addEntry(username, salt, sha256(password+salt));
			...
			case 3:	cout << "enter your username:  ";
					...
					if(myHashTable->removeUser(username, sha256((password) + (myHashTable->getSalt(username)))))//remove the user from the table and check if they were removed successfully----
